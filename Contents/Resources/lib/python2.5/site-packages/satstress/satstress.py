...
class PW_Variables(object): #
    """
        A container class for the necessary Polar Wander coordinates.
        """
    
    def __init__(self, thetaRi=0, phiRi=0, thetaRf=0, phiRf=0, thetaTi=0, phiTi=0, thetaTf=0, phiTf=0):
        self.thetaRInitial = numpy.radians(thetaRi) + scipy.pi/2
        self.phiRInitial = numpy.radians(phiRi)
        self.thetaRFinal = numpy.radians(thetaRf) + scipy.pi/2
        self.phiRFinal = numpy.radians(phiRf)
        self.thetaTInitial = numpy.radians(thetaTi) + scipy.pi/2
        ...
        self.thetaTFinal = self.thetaTInitial + DeltaTheta
        self.phiTFinal = self.phiTInitial
    
    def __str__(self):
        return("thetaRInitial = %s, phiRInitial = %s\nthetaRFinal = %s, phiRFinal = %s\nthetaTInitial = %s, phiTInitial = %s\nthetaTFinal = %s, phiTFinal = %s" %
               (self.thetaRInitial, self.phiRInitial, self.thetaRFinal, self.phiRFinal, self.thetaTInitial, self.phiTInitial, self.thetaTFinal, self.phiTFinal))
    ...
    def update_phiTi(self, phiTi):
        self.phiTInitial = numpy.radians(phiTi)
    #DeltaPhi = self.phiRFinal - self.phiRInitial
    #self.phiTFinal = self.phiTInitial + DeltaPhi
...
class PolarWander(StressDef): #{{{
    ...
    def __init__(self, satellite):
        ...
        self.omega = self.satellite.mean_motion()
        #This equation is supposed to use an elastic case, so I am unsure if this is what we should use as omega.
        ...
        Omega = (2.0 * scipy.pi) / self.satellite.orbit_period()
        self.Flattening = ((self.satellite.radius())**3 * Omega**2) / (2.0 * pc.G * self.satellite.mass()) * self.love.h2.real
        ...
        """
        self.PWC = self.Flattening * self.satellite.layers[3].lame_mu * (1.0 + self.satellite.layers[3].poissons_ratio)/(5.0 + self.satellite.layers[3].poissons_ratio)
        """
        This is a constant which is used in all of the stress calculations.  I just took it out to be calculated separately.
        """
        self.Number_of_increments = 1
        self.thetaRStep = (self.PWCoordinates.thetaRFinal - self.PWCoordinates.thetaRInitial) / self.Number_of_increments
        self.phiRStep = (self.PWCoordinates.phiRFinal - self.PWCoordinates.phiRInitial) / self.Number_of_increments
        
        self.thetaTStep = (self.PWCoordinates.thetaTFinal - self.PWCoordinates.thetaTInitial) / self.Number_of_increments
        self.phiTStep = (self.PWCoordinates.phiTFinal - self.PWCoordinates.phiTInitial) / self.Number_of_increments
    ...
    def Ttt(self, theta, phi, t):
        ...
        theta_Ri = self.PWCoordinates.thetaRInitial #The initial position of the pole
        phi_Ri = self.PWCoordinates.phiRInitial
        theta_Rf = self.PWCoordinates.thetaRFinal
        phi_Rf = self.PWCoordinates.phiRFinal
        #theta_Rf = self.thetaRStep * t #The current position of the pole
        #phi_Rf = self.phiRStep * t
                
        theta_Ti = self.PWCoordinates.thetaTInitial #The initial position of the sub-Jove point
        phi_Ti = self.PWCoordinates.phiTInitial
        theta_Tf = self.PWCoordinates.thetaTFinal
        phi_Tf = self.PWCoordinates.phiTFinal
        #theta_Tf = self.thetaTStep * t #The current position of the sub-Jove point
        #phi_Tf = self.phiTStep * t

        RDi = (6 * (self.SinGammaCosPsi(theta, phi, theta_Ri, phi_Ri))**2) + (9 * self.CosGamma(theta, phi, theta_Ri, phi_Ri)**2) - 5 #Calculates the stress from the initial position of the pole
        RDf = (6 * (self.SinGammaCosPsi(theta, phi, theta_Rf, phi_Rf))**2) + (9 * self.CosGamma(theta, phi, theta_Rf, phi_Rf)**2) - 5 #Calculates the stress from the reorientation of the pole
        TDi = (6 * (self.SinGammaCosPsi(theta, phi, theta_Ti, phi_Ti))**2) + (9 * self.CosGamma(theta, phi, theta_Ti, phi_Ti)**2) - 5 #Calculates the stress from the initial tidal deformation
        TDf = (6 * (self.SinGammaCosPsi(theta, phi, theta_Tf, phi_Tf))**2) + (9 * self.CosGamma(theta, phi, theta_Tf, phi_Tf)**2) - 5 #Calculates the stress from the final tidal deformation
    ...
    def Tpp(self, theta, phi, t):
        ...
        RDi = (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Ri, phi_Ri))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Ri, phi_Ri))**2) + 1.0 #Calculates the stress from the initial position of the pole
        RDf = (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Rf, phi_Rf))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Rf, phi_Rf))**2) + 1.0 #Calculates the stress from the reorientation of the pole
        TDi = (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Ti, phi_Ti))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Ti, phi_Ti))**2) + 1.0 #Calculates the stress from the initial tidal deformation
        TDf = (-6.0 * (self.SinGammaCosPsi(theta, phi, theta_Tf, phi_Tf))**2) + (3.0 * (self.CosGamma(theta, phi, theta_Tf, phi_Tf))**2) + 1.0 #Calculates the stress from the final tidal deformation   
    ...
    def Tpt(self, theta, phi, t):
        ...
        RDi = 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Ri, phi_Ri)) * (self.SinGammaCosPsi(theta, phi, theta_Ri, phi_Ri)) #Calculates the stress from the initial position of the pole
        RDf = 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Rf, phi_Rf)) * (self.SinGammaCosPsi(theta, phi, theta_Rf, phi_Rf)) #Calculates the stress from the reorientation of the pole
        TDi = 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Ti, phi_Ti)) * (self.SinGammaCosPsi(theta, phi, theta_Ti, phi_Ti)) #Calculates the stress from the initial tidal deformation
        TDf = 2.0 * (self.SinGammaSinPsi(theta, phi, theta_Tf, phi_Tf)) * (self.SinGammaCosPsi(theta, phi, theta_Tf, phi_Tf)) #Calculates the stress from the final tidal deformation   
