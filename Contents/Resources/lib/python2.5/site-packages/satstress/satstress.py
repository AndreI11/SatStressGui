...
class Satellite(object):
    ...
    def __init__(self, satParams, satFile=None): #
        """
...
163		Node number of 3rd innermost boundary""" % (self.satellite.density()/1000.0,\
            ...
            #print h2_real, h2_imag, k2_real, k2_imag, l2_real, l2_imag
...
class Diurnal(StressDef): #{{{
    ...
    def __init__(self, satellite):
        ...
        #print 'scparameters', self.scparameters
...
class PolarWander(StressDef): #{{{
    ...
    def __init__(self, satellite):
        ...
        Omega = 2.0 * scipy.pi * self.orbit_period()
        self.Flattening = ((self.radius())**3 * Omega**2) / (2.0 * pc.G * self.mass()) * self.h2.real
        """
        ...
        """
        self.PWC = self.Flattening * self.satellite.layers[4].lame_mu * (1.0 + self.satellite.layers[4].poissons_ratio)/(5.0 + self.satellite.layers[4].poissons_ratio)
        

        self.thetaRInitial += scipy.pi/2 #Converts to colatitude
        self.thetaRFinal += scipy.pi/2 #Converts to colatitude
        self.Number_of_increments = 1
        self.thetaRStep = (self.thetaRFinal - self.thetaRInitial) / self.Number_of_increments
        self.phiRStep = (self.phiRFinal - self.phiRInitial) / self.Number_of_increments
        
        
        DeltaTheta = self.thetaRFinal - self.thetaRInitial
        DeltaPhi = self.phiRFinal - self.phiRInitial
        
        self.thetaTInitial += scipy.pi/2 #Converts to colatitude
        self.thetaTFinal += scipy.pi/2 #Converts to colatitude
        self.phiTInitial = self.phiRInitial + DeltaTheta
        self.phiTFinal = self.phiRFinal + DeltaPhi
        self.thetaTStep = (self.thetaTFinal - self.thetaTInitial) / self.Number_of_increments
        self.pHITStep = (self.phiTFinal - self.phiTInitial) / self.Number_of_increments
    ...
    def Ttt(self, theta, phi, t):
        ...
        theta_Ri = self.thetaRInitial #The initial position of the pole
        phi_Ri = self.phiRInitial
        theta_Rf = self.thetaRStep * t #The current position of the pole
        phi_Rf = self.phiRStep * t
        
        theta_Ti = self.thetaTInitial #The initial position of the sub-Jove point
        phi_Ti = self.phiTInitial
        theta_Tf = self.thetaTStep * t #The current position of the sub-Jove point
        phi_Tf = self.phiTStep * t
        
        RDi = (6 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Ri, phi_Ri - scipy.py/2))**2) + (9 * self.CosGamma(theta, phi - scipy.py/2, theta_Ri, phi_Ri - scipy.py/2)**2) - 5 #Calculates the stress from the initial position of the pole
        RDf = (6 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Rf, phi_Rf - scipy.py/2))**2) + (9 * self.CosGamma(theta, phi - scipy.py/2, theta_Rf, phi_Rf - scipy.py/2)**2) - 5 #Calculates the stress from the reorientation of the pole
        TDi = (6 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Ti, phi_Ti - scipy.py/2))**2) + (9 * self.CosGamma(theta, phi - scipy.py/2, theta_Ti, phi_Ti - scipy.py/2)**2) - 5 #Calculates the stress from the initial tidal deformation
        TDf = (6 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Tf, phi_Tf - scipy.py/2))**2) + (9 * self.CosGamma(theta, phi - scipy.py/2, theta_Tf, phi_Tf - scipy.py/2)**2) - 5 #Calculates the stress from the final tidal deformation
   
        """
        In order to calculate the stresses correctly, the coordinates have to be transformed from a reference frame
        where the rotational pole moves and the shell is stationary to one in which the rotational pole is
        stationary and the shell moves.  To do this, all of the phi values are decreased by pi/2 (180 degrees).
        The colatitude is unchanged.
        Physically, this would be the ice shell reorienting itself by moving over the core, which remains unchanged.

        See Matsuyama and Nimmo 2007, "Tectonic patterns on reoriented and despun planetary bodies".
        """     

        TttPW = self.PWC * (2.0/3.0) * (RDi - RDf - 3.0 * TDi + 3.0 * TDf)
    ...
    def Tpp(self, theta, phi, t):
        ...
        RDi = (-6.0 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Ri, phi_Ri - scipy.py/2))**2) + (3.0 * (self.CosGamma(theta, phi - scipy.py/2, theta_Ri, phi_Ri - scipy.py/2))**2) + 1.0 #Calculates the stress from the initial position of the pole
        RDf = (-6.0 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Rf, phi_Rf - scipy.py/2))**2) + (3.0 * (self.CosGamma(theta, phi - scipy.py/2, theta_Rf, phi_Rf - scipy.py/2))**2) + 1.0 #Calculates the stress from the reorientation of the pole
        TDi = (-6.0 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Ti, phi_Ti - scipy.py/2))**2) + (3.0 * (self.CosGamma(theta, phi - scipy.py/2, theta_Ti, phi_Ti - scipy.py/2))**2) + 1.0 #Calculates the stress from the initial tidal deformation
        TDf = (-6.0 * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Tf, phi_Tf - scipy.py/2))**2) + (3.0 * (self.CosGamma(theta, phi - scipy.py/2, theta_Tf, phi_Tf - scipy.py/2))**2) + 1.0 #Calculates the stress from the final tidal deformation
        ...
        TppPW = self.PWC * (2.0/3.0) * (RDi - RDf - 3.0 * TDi + 3.0 * TDf)
    ...
    def Tpt(self, theta, phi, t):
        ...
        theta_Ri = self.thetaRInitial #The initial position of the pole
        phi_Ri = self.phiRInitial
        theta_Rf = self.thetaRStep * t #The final position of the pole
        phi_Rf = self.phiRStep * t
        ...
        RDi = 2.0 * (self.SinGammaSinPsi(theta, phi - scipy.py/2, theta_Ri, phi_Ri - scipy.py/2)) * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Ri, phi_Ri - scipy.py/2)) #Calculates the stress from the initial position of the pole
        RDf = 2.0 * (self.SinGammaSinPsi(theta, phi - scipy.py/2, theta_Rf, phi_Rf - scipy.py/2)) * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Rf, phi_Rf - scipy.py/2)) #Calculates the stress from the reorientation of the pole
        TDi = 2.0 * (self.SinGammaSinPsi(theta, phi - scipy.py/2, theta_Ti, phi_Ti - scipy.py/2)) * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Ti, phi_Ti - scipy.py/2)) #Calculates the stress from the initial tidal deformation
        TDf = 2.0 * (self.SinGammaSinPsi(theta, phi - scipy.py/2, theta_Tf, phi_Tf - scipy.py/2)) * (self.SinGammaCosPsi(theta, phi - scipy.py/2, theta_Tf, phi_Tf - scipy.py/2)) #Calculates the stress from the final tidal deformation
        ...
        TptPW = self.PWC * (-2.0) * (RDi - RDf - 3.0 * TDi + 3.0 * TDf)  
...
class StressCalc(object): #{{{
    ...
    def principal_components(self, theta, phi, t): #{{{2
        ...
        #print eigval_A, theta_A, phi_A, eigval_B, theta_B, phi_B
