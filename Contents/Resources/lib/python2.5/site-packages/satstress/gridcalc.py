...
class Grid(Model.DataModel): # {{{
    ...
    def __init__(self,satellite, gridParams):
        ...
        print self.parameters['LAT_NUM'].value
        for param in self.get_parameters_by_category('grid_var'):
            if param.name in ['LAT_MIN', 'LON_MIN', 'LON_NUM','LAT_NUM', 'LAT_MAX','LON_MAX','ORBIT_MIN','ORBIT_MAX','ORBIT_NUM']:
                print param.name
                if not param.value:
                    raise MissingDimensionError(gridfile, param.name )
                param.set_param_type(float)
    ...
    def __str__(self):
        ...
        if self.parameters['ORBIT_MIN'] is None:
            time_comment = '#'
            orbit_comment = ''
        else:
            time_comment = ''
...
%sORBIT_NUM = %s
        """ % (self.grid_id,\
               self.parameters['LAT_MIN'], self.parameters['LAT_MAX'], self.parameters['LAT_NUM'],\
               self.parameters['LON_MIN'], self.parameters['LON_MAX'], self.parameters['LON_NUM'],\
               time_comment, self.parameters['TIME_MIN'],\
               time_comment, self.parameters['TIME_MAX'],\
               time_comment, self.parameters['TIME_NUM'],\
               orbit_comment, self.parameters['ORBIT_MIN'],\
               orbit_comment, self.parameters['ORBIT_MAX'],\
               orbit_comment, self.parameters['ORBIT_NUM'],)
...
NSR_PERIOD_MAX = %s
NSR_PERIOD_NUM = %s
        """ % (self.parameters['NSR_PERIOD'], self.parameters['NSR_PERIOD'], self.nsr_period_num)
...
""" % (self.missing_dim, self.gridfile))
