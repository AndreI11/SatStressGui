...
class Cycloid(object):
    ...
    """
    def __init__(self, stresscalc, threshold, propagation_strength, propagation_speed,
    ...
    def propagate_split(self, prevpt_data):
        ...
        """
        while self.traveled <= self.current_orbit_pos and not self.stop_propgating:
            ...
            # print prevlat, prevlon, s1, s1az, s3, s3az
    ...
    def plotcoordsonbasemap(self,basemap_ax, orbit_pos):
        """
        ...
        if self.initiated:
            self.propagate_split(self.pt_data)
        x,y = basemap_ax(self.lon_cycloidpts[:int(3*orbit_pos/self.degree_step)], self.lat_cycloidpts[:int(3*orbit_pos/self.degree_step)]) #3 because thats how many points are added per degree_step
